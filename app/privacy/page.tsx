  ...
  return (
    <LegalLayout title="Privacy Policy" updated="September 2026">
      ...
      <ul>
        ...
        <li>
          <strong>A random visitor identifier</strong> stored in the visitor&apos;s own browser
          (local storage). It is generated by us, is not derived from any personal information,
          and is not linked to a Shopify customer account. We use it to avoid showing the same
          person the same offer repeatedly and to keep measurement groups stable across visits.
          A visitor can remove it by clearing their browser storage, and if storage is
          unavailable we simply proceed without one.
        </li>
      </ul>
      ...
      <h2>Measurement and Controlled Testing</h2>
      <p>
        To measure whether our app actually increases a store&apos;s sales, rather than simply
        claiming it does, we hold back a randomly selected share of store visitors and show them
        nothing at all. We then compare how often each group completes a purchase. This is the
        only way to tell a real effect apart from sales that would have happened anyway.
      </p>
      <ul>
        <li>
          <strong>Who is held back:</strong> currently 10% of eligible visitors, selected by a
          mathematical hash of the random visitor identifier described above. The same visitor
          stays in the same group across visits.
        </li>
        <li>
          <strong>What it means for that visitor:</strong> they see no modal and no offer. We
          collect <em>less</em> information about them than about other visitors, not more.
        </li>
        <li>
          <strong>How results are used:</strong> only as counts and rates for each group. We do
          not build profiles of individual shoppers, and no decision with a legal or similarly
          significant effect on any person is made from this data.
        </li>
      </ul>
      <p>
        We may change the size of the held-back group over time as a store&apos;s traffic grows.
        The purpose and the categories of data involved do not change when we do.
      </p>
